...
function mis = simulations(name, n, dim, sigma, K, p)
...
%MissrateSSC = missclassify_heuristic(Grps, N, K);
GrpsSSC = Grps;
...
CKSym = abs(C);
[Grps , SingVals, LapKernel] = SpectralClustering(CKSym,K);
%MissrateLR = missclassify_heuristic(Grps, N, K);

%mis=[MissrateSSC, MissrateLR];

filename = ['../../' name '/segmentationMATLAB', num2str(dim) '.csv'];
